...
bool SimpleTracker::isTracked(int userId)
{
    ...
    frame->getUsers()[userId].getSkeleton().getState() == SKELETON_TRACKED;
}
...
const nite::Skeleton& SimpleTracker::getSkeleton(int userId) {
    return frame->getUsers()[userId].getSkeleton();
}